...
export default function HomeComponent() {
  ...
  // Generate random viewer count between 900,000 and 1,000,000
  useEffect(() => {
  ...
  const fetchComments = async () => {
    if (!latestStream?.muxStreamId) {
      console.log('HomeComponent - No muxStreamId available yet');
      return;
    }
    
    console.log('HomeComponent - Fetching comments for muxStreamId:', latestStream.muxStreamId);
    
    try {
      const response = await fetch(`/api/comments/list?streamId=${latestStream.muxStreamId}`);
      console.log('HomeComponent - Comments API response status:', response.status);
      if (response.ok) {
        const data = await response.json();
        console.log('HomeComponent - Fetched comments count:', data.comments.length);
        console.log('HomeComponent - First few comments:', data.comments.slice(0, 3));
        setComments(data.comments);
      } else {
        console.error('HomeComponent - Comments API error:', response.statusText);
      }
    } catch (error) {
                  ...
                  {comments.length > 0 ? (
                    comments.map((comment) => (
                      <div key={comment._id} className="text-xs">
                        ...
                        <p className="text-white/90 mt-0.5">{comment.text}</p>
                      </div>
                    ))
                  ) : (
